...
OPENCV_STREAM_URL = os.getenv("OPENCV_STREAM_URL", "http://ai_detection:5001/stream")
...
def on_faces_message(client, userdata, msg):
    ...
    try:
        ...
        print(f"[WEB] Erreur parsing faces: {error}", flush=True)
...
try:
    ...
    print(f"[WEB] MQTT indisponible: {error}", flush=True)
...
def proxy_stream():
    with requests.get(OPENCV_STREAM_URL, stream=True, timeout=10) as response:
